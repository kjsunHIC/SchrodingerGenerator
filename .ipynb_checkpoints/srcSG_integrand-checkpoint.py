...
import math
import numpy as np
from srcSG_utilities import    to_tensor
...
def fun_ws(x: torch.Tensor, param: dict):
    ...
    if Inucleus == 208:
        ...
        R0 = torch.tensor(5.02, device=device, dtype=dtype, requires_grad=False)
        w  = torch.tensor(0.0, device=device, dtype=dtype, requires_grad=False)
        aa = torch.tensor(0.52, device=device, dtype=dtype, requires_grad=False)
        beta2 = torch.tensor(0.06, device=device, dtype=dtype, requires_grad=False)
        beta3 = torch.tensor(0.20, device=device, dtype=dtype, requires_grad=False)
        beta4 = torch.tensor(0.0, device=device, dtype=dtype, requires_grad=False)
        gamma = torch.tensor(0.0, device=device, dtype=dtype, requires_grad=False)
        Nor = torch.tensor(2.4932e4, device=device, dtype=dtype, requires_grad=False) # needs to be determined
 
    if Inucleus == 20:
        R0 = torch.tensor(2.608, device=device, dtype=dtype, requires_grad=False)
        w  = torch.tensor(-0.168, device=device, dtype=dtype, requires_grad=False)
        aa = torch.tensor(0.698, device=device, dtype=dtype, requires_grad=False)
        beta2 = torch.tensor(0.0, device=device, dtype=dtype, requires_grad=False)
        beta3 = torch.tensor(0.0, device=device, dtype=dtype, requires_grad=False)
        beta4 = torch.tensor(0.0, device=device, dtype=dtype, requires_grad=False)
        gamma = torch.tensor(0.0, device=device, dtype=dtype, requires_grad=False)
    ...
    # Jastrow
    #term = 1 + a * torch.exp(-b * rij**2) * (c + d * rij**2 + e * rij**3 + f * rij**4)
    # in paper, g(\Delta r)=1-e^{-\alpha(\Delta r)^2} (\gamma+\sum_{i=1}^{3}\beta_i(\Delta r)^{i+1} )
    
    if Isrc == 1: #Alvioli-AV8′
        if nd <193:
            J, log_J = calculate_jastrow_factor(X, -1,1.2627,0.9916,2.1032,-6.1297,2.9348) # ( ,alpha,gamma, beta1, beta2, beta3)
        else:
            J, log_J = calculate_jastrow_factor_streaming(X, -1,1.2627,0.9916,2.1032,-6.1297,2.9348)
    elif Isrc == 2: #CD-Bonn
        if nd <193:
            J, log_J = calculate_jastrow_factor(X, -1,1.2746,0.3820,-0.8557,0.1328,0.0849)
        else:
            J, log_J = calculate_jastrow_factor_streaming(X, -1,1.2746,0.3820,-0.8557,0.1328,0.0849)
    elif Isrc == 3: #Simkovic
        if nd <193:
            J, log_J = calculate_jastrow_factor(X, -1,1.9319,0.9868,2.5145,-4.7581,0.0555)
        else:
            J, log_J = calculate_jastrow_factor_streaming(X, -1,1.9319,0.9868,2.5145,-4.7581,0.0555)
    elif Isrc == 4:#Miller
        if nd>193:
            J, log_J = calculate_jastrow_factor(X, -1,1.3977,0.9965,1.8195,-1.6541,-0.3312)
        else:
...
#def fun_cluster_torch(x: torch.Tensor, param: dict) -> torch.Tensor:
    ...
    nd = int(param['nd'])
    scl = param['scale']
    a = to_tensor(param['xmin'], dtype=dtype, device=device)[:nd]
    b = to_tensor(param['xmax'], dtype=dtype, device=device)[:nd]
